...
// =====================================================
// === Météo AROME via Open-Meteo ====================
// =====================================================

const WMO_LABELS = {
  0:'Ciel dégagé', 1:'Partiellement clair', 2:'Partiellement nuageux', 3:'Couvert',
  45:'Brouillard', 48:'Brouillard givrant',
  51:'Bruine légère', 53:'Bruine', 55:'Bruine forte',
  56:'Bruine verg. légère', 57:'Bruine verglaçante',
  61:'Pluie faible', 63:'Pluie', 65:'Pluie forte',
  66:'Pluie verg. légère', 67:'Pluie verglaçante',
...
function wmoLabel(code){ return WMO_LABELS[code] || `Code météo ${code}`; }
...
async function getGPSCoords(){
  return new Promise((res, rej) => {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      status('Position obtenue: ' + currentCoords.lat.toFixed(5) + ', ' + currentCoords.lon.toFixed(5));
      res(currentCoords);
    }, err => rej(new Error(err.message)), { enableHighAccuracy: true, timeout: 15000 });
  });
}

async function fetchAromeMeteo(lat, lon){
  const url = 'https://api.open-meteo.com/v1/meteofrance'
    + '?latitude=' + lat.toFixed(5)
    + '&longitude=' + lon.toFixed(5)
    + '&current=temperature_2m,precipitation,weather_code,wind_speed_10m,wind_direction_10m,snowfall,snow_depth'
    + '&hourly=temperature_2m,precipitation,snowfall,snow_depth,wind_speed_10m,wind_direction_10m,weather_code'
...
function renderMeteo(data){
  const meteoSection = document.getElementById('meteo-section');
  const currentDiv = document.getElementById('meteo-current');
  const forecastDiv = document.getElementById('meteo-forecast');
  meteoSection.classList.remove('hidden');

  const c = data.current;
  const wcode = c.weather_code;
  const snowDepthCm = c.snow_depth > 0 ? (c.snow_depth * 100).toFixed(0) : 0;

  let detailsHtml = `<span>💨 ${c.wind_speed_10m} km/h ${degToCompass(c.wind_direction_10m)}</span>`;
  detailsHtml += `<span>🌧️ ${c.precipitation} mm</span>`;
  if (c.snowfall > 0) detailsHtml += `<span>🌨️ ${c.snowfall} cm fraîche</span>`;
  if (snowDepthCm > 0) detailsHtml += `<span>❄️ ${snowDepthCm} cm sol</span>`;

  currentDiv.innerHTML =
    '<div class="meteo-header">'
    + '<span class="meteo-icon-big">' + wmoIcon(wcode) + '</span>'
    + '<span class="meteo-temp">' + c.temperature_2m.toFixed(1) + '°C</span>'
    + '<span class="meteo-wmo-label">' + wmoLabel(wcode) + '</span>'
    + '</div>'
    + '<div class="meteo-details">' + detailsHtml + '</div>'
    + '<div class="meteo-updated">Actualisé : '
    + new Date(c.time).toLocaleString('fr-FR')
    + ' · Modèle AROME (Open-Meteo)</div>';
  ...
  const endIdx = Math.min(startIdx + 25, times.length);

  let hoursHtml = '';
  for (let i = startIdx; i < endIdx; i += 3){
    const t = new Date(times[i]);
    const hcode = data.hourly.weather_code[i];
    const snow = data.hourly.snowfall[i] || 0;
    const precip = data.hourly.precipitation[i] || 0;
    const hSnowDepth = data.hourly.snow_depth[i] || 0;
    const hWind = data.hourly.wind_speed_10m[i] || 0;
    const hTemp = data.hourly.temperature_2m[i];

    let extras = '';
    if (snow > 0) extras += '<div class="hour-snow">🌨️ ' + snow.toFixed(1) + 'cm</div>';
    else if (precip > 0) extras += '<div class="hour-precip">💧 ' + precip.toFixed(1) + 'mm</div>';
    if (hSnowDepth > 0) extras += '<div class="hour-depth">❄️ ' + (hSnowDepth * 100).toFixed(0) + 'cm</div>';

    hoursHtml +=
      '<div class="meteo-hour">'
      + '<div class="hour-time">' + t.toLocaleTimeString('fr-FR', {hour:'2-digit', minute:'2-digit'}) + '</div>'
      + '<div class="hour-date">' + t.toLocaleDateString('fr-FR', {weekday:'short', day:'numeric'}) + '</div>'
      + '<div class="hour-icon">' + wmoIcon(hcode) + '</div>'
      + '<div class="hour-temp">' + hTemp.toFixed(0) + '°C</div>'
      + extras
      + '<div class="hour-wind">💨 ' + hWind.toFixed(0) + ' km/h</div>'
      + '</div>';
  }

  forecastDiv.innerHTML =
    '<div class="meteo-forecast-title">Prévisions AROME — prochaines 24h</div>'
    + '<div class="meteo-forecast-row">' + hoursHtml + '</div>';
}

const getMeteoBtn = document.getElementById('getMeteoBtn');

getMeteoBtn.addEventListener('click', async () => {
  const meteoSection = document.getElementById('meteo-section');
  const meteoLoading = document.getElementById('meteo-loading');
  const currentDiv = document.getElementById('meteo-current');
  const forecastDiv = document.getElementById('meteo-forecast');

  meteoSection.classList.remove('hidden');
  meteoLoading.classList.remove('hidden');
  currentDiv.innerHTML = '';
  forecastDiv.innerHTML = '';

  try {
    if (!currentCoords) {
      status('Obtention position pour météo...');
      await getGPSCoords();
    }
    const data = await fetchAromeMeteo(currentCoords.lat, currentCoords.lon);
    meteoLoading.classList.add('hidden');
    renderMeteo(data);
    status('Météo AROME chargée');
  } catch(err) {
    meteoLoading.classList.add('hidden');
    currentDiv.innerHTML = '<p class="meteo-error">Erreur météo : ' + err.message + '</p>';
    status('Erreur météo AROME');
  }
});
